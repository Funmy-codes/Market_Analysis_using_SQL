...
SELECT * FROM CUSTOMER
WHERE WORKING_AREA = 'New York' OR WORKING_AREA = 'Mumbai' OR WORKING_AREA = 'San Jose'


SELECT * FROM CUSTOMER
WHERE WORKING_AREA = 'USA' OR WORKING_AREA = 'Canada'
...
SELECT CUST_NAME, WORKING_AREA, WORKING_AREA, OUTSTANDING_AMT FROM CUSTOMER
ORDER BY OUTSTANDING_AMT DESC

SELECT * FROM CUSTOMER
WHERE WORKING_AREA = 'New York'

SELECT WORKING_AREA, AVG(OUTSTANDING_AMT) AS Average_debt FROM CUSTOMER
WHERE WORKING_AREA = 'USA'
GROUP BY WORKING_AREA


SELECT SUM(RECEIVE_AMT + PAYMENT_AMT) AS sum_of_received_and_paymentamount FROM CUSTOMER

SELECT CUST_NAME, WORKING_AREA, OPENING_AMT, RECEIVE_AMT, PAYMENT_AMT FROM CUSTOMER
ORDER BY OPENING_AMT DESC

SELECT TOP(3) CUST_NAME, WORKING_AREA, PAYMENT_AMT FROM CUSTOMER
ORDER BY PAYMENT_AMT DESC

SELECT TOP(3) WORKING_AREA, SUM(PAYMENT_AMT) AS sum_of_payment_amount FROM CUSTOMER
group by WORKING_AREA
ORDER BY sum_of_payment_amount DESC

-------ASSIGNMENT
-- which city often time have conversions (in terms of opening amount)
SELECT TOP(3) CUST_CITY, AVG(OPENING_AMT) AS average_of_openingamount FROM CUSTOMER
GROUP BY CUST_CITY
ORDER BY average_of_openingamount DESC

-- Info on the city with that receives payment the most
SELECT TOP(5) WORKING_AREA, SUM(RECEIVE_AMT) AS sum_of_received_amount FROM CUSTOMER
GROUP BY WORKING_AREA
ORDER BY sum_of_received_amount DESC


-- what working area has the most customer base
SELECT WORKING_AREA, COUNT(WORKING_AREA) AS no_of_people FROM CUSTOMER
GROUP BY WORKING_AREA
ORDER BY no_of_people DESC


-- chose what city to avoid due to rising debt
SELECT TOP(3) CUST_CITY, SUM(OUTSTANDING_AMT) AS outstanding_amount FROM CUSTOMER
GROUP BY CUST_CITY
ORDER BY outstanding_amount DESC

-- WHAT COUNTRY MAKES THE MOST MONEY (sum all amount columns  and deduct by the debt amount)
SELECT TOP(3) CUST_COUNTRY, SUM((RECEIVE_AMT + PAYMENT_AMT + OPENING_AMT) - OUTSTANDING_AMT) AS total_amount FROM CUSTOMER
GROUP BY CUST_COUNTRY
ORDER BY total_amount DESC


-- number of customers in this country above
SELECT CUST_COUNTRY, COUNT(CUST_NAME) AS no_of_customer FROM CUSTOMER
...
------------------------
SELECT CUST_NAME, WORKING_AREA, OPENING_AMT FROM CUSTOMER
WHERE OPENING_AMT BETWEEN 5000 AND 8000
...
--- The customer that has the lowest amount and the customer that has the highest grade, are they the same?
SELECT CUSTOMER.CUST_NAME, CUSTOMER.GRADE, ORDERS.ORD_AMOUNT FROM CUSTOMER
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
ORDER BY GRADE DESC
...
RIGHT JOIN CUSTOMER ON CUSTOMER.AGENT_CODE = AGENTS.AGENT_CODE
ORDER BY RECEIVE_AMT DESC

-- What is the order number of all sales made on 2008-08-01, and show the information of the agent and customer that is involved in this sales
SELECT ORDERS.ORD_NUM, ORDERS.ORD_DATE, AGENTS.AGENT_CODE, AGENTS.AGENT_NAME, AGENTS.WORKING_AREA, CUSTOMER.CUST_NAME, CUSTOMER.CUST_CODE FROM CUSTOMER
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
...
SELECT AGENTS.*, CUSTOMER.CUST_COUNTRY, CUSTOMER.CUST_NAME FROM CUSTOMER
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
WHERE CUST_COUNTRY = 'USA'

-- Who is our best customer (Total of all amounts less outstanding), what date did he purchase, and who is their agent?
SELECT TOP(5) CUSTOMER.CUST_NAME,
SUM((CUSTOMER.OPENING_AMT + CUSTOMER.RECEIVE_AMT + CUSTOMER.PAYMENT_AMT) - CUSTOMER.OUTSTANDING_AMT) AS Total_amt,
ORDERS.ORD_DATE, AGENTS.AGENT_NAME FROM CUSTOMER
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
...
SELECT CUSTOMER.CUST_NAME, CUSTOMER.CUST_COUNTRY, AGENTS.AGENT_NAME, AGENTS.PHONE_NO, ORDERS.ORD_DATE FROM CUSTOMER
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
WHERE CUST_NAME = 'Micheal' AND CUST_COUNTRY = 'Torento'
...
SELECT AGENTS.*, CUSTOMER.CUST_NAME, CUSTOMER.PAYMENT_AMT, ORDERS.ORD_DATE FROM CUSTOMER
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
LEFT JOIN ORDERS ON ORDERS.CUST_CODE = CUSTOMER.CUST_CODE
WHERE PAYMENT_AMT > 6000


--- What agent brought the customer that has the highest openinng amount?
WITH SEUN AS(
SELECT CUSTOMER.CUST_NAME, CUSTOMER.OPENING_AMT, AGENTS.AGENT_NAME FROM CUSTOMER
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
) 

SELECT TOP(1) * FROM SEUN
ORDER BY OPENING_AMT DESC
...
SELECT TOP(1) * FROM ANS
ORDER BY ORD_AMOUNT DESC

---Show all information of agents that are from New York City
WITH ANSWER AS(
SELECT AGENTS.*, CUSTOMER.CUST_NAME, CUSTOMER.CUST_COUNTRY FROM CUSTOMER
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE
)

SELECT * FROM ANSWER
WHERE CUST_COUNTRY = 'New York'
...
-- Show the phone number of the agent that brought the customer with highest receive amount
WITH QUEST2 AS(
SELECT CUSTOMER.CUST_NAME, CUSTOMER.RECEIVE_AMT, AGENTS.AGENT_NAME, AGENTS.PHONE_NO FROM CUSTOMER
LEFT JOIN AGENTS ON AGENTS.AGENT_CODE = CUSTOMER.AGENT_CODE  
)
